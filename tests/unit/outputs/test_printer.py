...
@pytest.mark.parametrize(
    ...
    [
        (None, ["z_500", "cp", "tp"], []),
        ({"select": "cp"}, ["cp"], ["z_500", "tp"]),
        ({"select": ["z_500", "cp"]}, ["cp", "z_500"], ["tp"]),
        ({"drop": ["z", "cp"]}, ["tp", "z_500"], ["cp"]),
        ({"drop": "cp"}, ["tp", "z_500"], ["cp"]),
        ("all", ["z_500", "cp", "tp"], []),
    ],
)
def test_print_state_variable_inclusion(
    ...
    output = PrinterOutput(basic_context, basic_metadata, max_lines=0, variables=variables)
