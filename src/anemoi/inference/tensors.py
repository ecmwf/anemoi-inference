...
class TensorHandler:
    ...
    def add_dynamic_forcings_to_input_tensor(
        ...
        steady_state = any(
            self.context.development_hacks.get(name, False) for name in ("steady_state", "freeze_computed_forcings")
        )

        for source in self.dynamic_forcings_inputs:
            source_dates = dates
            if steady_state and source.kinds.get("computed", False):
                forecast_start = state["date"] - state.get("step", timedelta(0))
                source_dates = [forecast_start] * len(dates)
