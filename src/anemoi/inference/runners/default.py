...
class DefaultRunner(Runner):
    ...
    def create_boundary_forcings_input(self) -> Input:
        ...
        variables = self.variables.retrieved_prognostic_variables()
        config = (
            self._input_forcings("boundary_forcings", "-boundary", "forcings", "input")
            if variables
            else "empty"
        )
        input = create_input(
    ...
    def _combine_states(self, *states: dict[str, Any]) -> dict[str, Any]:
        ...
        for state in states[1:]:
            ...
            for name, values in itertools.chain(
                combined["fields"].items(), state.get("fields", {}).items()
            ):
                if shape is None:
                    shape = values.shape
            ...
            for key, value in state.items():
                ...
                if type(combined[key]) is not type(value):
                    raise ValueError(
                        f"Key '{key}' has different types in the states: "
                        f"{type(combined[key])} and {type(value)}."
                    )

                if isinstance(value, np.ndarray) and isinstance(
                    combined[key], np.ndarray
                ):
    ...
    def _initial_state(
        self,
        prognostic_state: dict[str, Any],
        constants_state: dict[str, Any],
        forcings_state: dict[str, Any],
    ) -> dict[str, Any]:
    ...
    def _check_state(self, state: dict[str, Any], title: str) -> None:
        ...
        if "fields" not in state:
            raise ValueError(
                f"State '{title}' does not contain 'fields': {state} ({input=})"
            )
        ...
        if date is None and len(state["fields"]) > 0:
            ...
            if not isinstance(date, datetime.datetime):
                raise ValueError(
                    f"State '{title}' does not contain 'date', or it is not a datetime: {date} ({input=})"
                )
