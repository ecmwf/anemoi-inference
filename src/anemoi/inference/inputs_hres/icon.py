...
@input_hres_registry.register("icon_grib_file")
class IconInput(GribInput):
    ...
    def create_input_state(self, *, date):
        ...
        latitudes = np.rad2deg(
            ds.clat[ds.refinement_level_c <= self.refinement_level_c].values
        )
        longitudes = np.rad2deg(
            ds.clon[ds.refinement_level_c <= self.refinement_level_c].values
        )
    ...
    def load_forcings(self, *, variables, dates):
        return self._load_forcings(
            ekd.from_source("file", self.path), variables=variables, dates=dates
        )
