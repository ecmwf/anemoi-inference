...
class EkdInput(Input):
    ...
    def _create_input_state(
        ...
        *,
        ...
        input_state = dict(
            date=date, latitudes=latitudes, longitudes=longitudes, fields=dict()
        )

        fields = input_state["fields"]

        input_fields = self._filter_and_sort(
            input_fields, variables=variables, dates=dates, title="Create input state"
        )
        ...
        for field in input_fields:
            ...
            name, valid_datetime = field.metadata("name"), field.metadata(
                "valid_datetime"
            )
            ...
            try:
                fields[name][date_idx] = mask.apply(
                    field.to_numpy(dtype=dtype, flatten=flatten)
                )
            except ValueError:
                LOG.error(
    ...
    def _load_forcings(self, fields, variables, dates):
        data = self._filter_and_sort(
            fields, variables=variables, dates=dates, title="Load forcings"
        )
        return data.to_numpy(dtype=np.float32, flatten=True).reshape(
            len(variables), len(dates), -1
        )
